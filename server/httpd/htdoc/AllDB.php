...
function calculateDeadlines($start, $studentType, $enrolmentTypeChangeList, $suspensionsList) {
	...
	for($i = 0; $i < $typeChangelength; $i ++) {
		...
		if ($startFTE <= $proposalFTE && (is_null($endFTE) || $proposalFTE < $endFTE)) {
		...
		if ($studentType === "Masters" && $startFTE <= $month3FTE && (is_null($endFTE) || $month3FTE < $endFTE)) {
		...
		if ($studentType === "Masters" && $startFTE <= $month8FTE && (is_null($endFTE) || $month8FTE < $endFTE)) {
		...
		if ($startFTE <= $thesisFTE && (is_null($endFTE) || $thesisFTE < $endFTE)) {
